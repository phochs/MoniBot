...
class Webserver {
     ...
     * @param {function} callback - Called when the web server has started
     */
    constructor(guilds, config, callback) {
        this.app = express();
        this.guilds = guilds;
        this.config = config;
        ...
        this.app.listen(this.config.config.port, () => {
            console.log(`App is listening on port ${this.config.config.port}`);
